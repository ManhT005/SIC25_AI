...
df = pd.read_csv(r"data\hanoi_daily_052025.csv")
df = df.rename(columns={"Unnamed: 0": "date"})  # Đổi tên cột ngày cho rõ ràng

cols_to_fill = ['pm25', 'pm10', 'co', 'no2', 'so2', 'o3']
for col in cols_to_fill:
    df[col] = df[col].fillna(df[col].rolling(window=5, min_periods=1, center=True).mean())
...
# ======== 3. Tính AQI cho từng chất theo chuẩn US EPA ========
...
df.to_csv(r"mined data\hanoi_daily_052025.csv", index=False)
